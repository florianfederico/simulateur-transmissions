...
public class RecepteurNRZTTest {
	...
	private AFactoryEmetteurRecepteur factory = new FactoryNRZT();

	/**
	 * Test method for
	 * {@link emetteurRecepteur.recepteur.impl.RecepteurNRZT#RecepteurNRZT()}.
	 */
	@Test
	public void testRecepteurNRZT() {
	...
	/**
	 * Test method for
	 * {@link emetteurRecepteur.recepteur.impl.RecepteurNRZT#CAN()}.
	 */
	@Test
	public void testCAN() {

		/*
		 * Le detecteur Equal ne peut pas detecter le signal NRZT
		 */
		/*
		 * EnumDetecteur detecteur = EnumDetecteur.EQUAL;
		 * assertTrue(this.testTransmissionParfaite(detecteur, 30, 5, 100) ==
		 * true); assertTrue(this.testTransmissionParfaite(detecteur, 30, 0, 10)
		 * == true); assertTrue(this.testTransmissionParfaite(detecteur, 30,
		 * -100, 0) == true);
		 * assertTrue(this.testTransmissionParfaite(detecteur, 30, -100, 100) ==
		 * true); assertTrue(this.testTransmissionParfaite(detecteur, 30, -100,
		 * -10) == true);
		 */

		EnumDetecteur detecteur = EnumDetecteur.FILTRE_ADAPTE;
	...
	private boolean testTransmissionParfaite(EnumDetecteur detecteurType, int ne, float aMin, float aMax) {
		...
		AEmetteur emetteur = new EmetteurNRZT(ne, aMin, aMax);
